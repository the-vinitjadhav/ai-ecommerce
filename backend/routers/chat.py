...
def get_product_recommendation(search_term: str = None, max_price: int = None, category: str = None, user_id: int = 0) -> str:
    ...
    try:
        ...
        if not products: return "Catalog is currently empty."
...
def compare_products(product_a: str, product_b: str, user_id: int = 0) -> str:
    ...
    try:
        ...
        if len(results) < 2: return f"Could not find both items to compare ('{product_a}' and '{product_b}')."
...
def get_product_details(product_name: str, user_id: int = 0) -> str:
    ...
    if not p: return f"Could not find product matching '{product_name}'."
    img = p.get('image_url') or f"https://ui-avatars.com/api/?name={urllib.parse.quote(str(p.get('product_name')))}&background=random"
    ...
    <div style="background: rgba(255,255,255,0.5); border: 1px solid rgba(0,0,0,0.05); border-radius: 12px; padding: 12px; margin-top: 5px;">
        ...
        <p style="font-size: 0.85rem; opacity: 0.8; margin-bottom: 12px; line-height: 1.4;">{p.get('description', 'Authentic product from AI Store.')}</p>
...
def find_cheaper_alternative(product_name: str, user_id: int = 0) -> str:
    ...
    if not target: 
        cursor.close(); close_db_connection(conn)
        return "Could not find the target product to match alternatives."
    ...
    if not alts: return f"No cheaper alternatives currently in {target['category_name']}."
...
def get_user_order_history(user_id: int) -> str:
    if user_id == 0: return "Please log in to view order history."
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""
        ...
        WHERE o.user_id = %s GROUP BY o.order_id ORDER BY o.order_date DESC LIMIT 5
    """, (user_id,))
    ...
    for o in orders:
        ...
        <div style="background: rgba(255,255,255,0.6); border-left: 4px solid #6366f1; padding: 10px 12px; border-radius: 0 8px 8px 0;">
            ...
            <div style="font-size: 0.8rem; color: #475569;">Items: {o.get('items_summary', 'Standard items')}</div>
...
def view_user_cart(user_id: int) -> str:
    if user_id == 0: return "Please log in to view cart."
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    html = "<p style='margin-bottom: 10px; font-size: 0.95rem;'>Items currently in your cart:</p><div style='display:flex; flex-direction:column; gap:8px;'>"
    for item in items:
        ...
        <div style="display: flex; gap: 12px; padding: 10px; background: rgba(255,255,255,0.6); border: 1px solid rgba(0,0,0,0.05); border-radius: 12px; align-items: center;">
            ...
            <div style="flex: 1;">
                <h6 style="margin: 0; font-size: 0.85rem; font-weight: bold;">{item['product_name']}</h6>
        ...
        """
    html += f"""
        <div style="margin-top: 8px; padding-top: 12px; border-top: 2px dashed rgba(0,0,0,0.1); display: flex; justify-content: space-between;">
            <b>Total:</b> <span style="font-weight: 900; color: #0f172a;">₹{total}</span>
        </div>
        <a href="cart.html" style="display: block; text-align: center; background: #6366f1; color: white; padding: 10px; border-radius: 8px; text-decoration: none; font-weight: bold; margin-top: 10px;">Go to Checkout</a>
    </div>
    """
    return html
...
def add_item_to_cart(user_id: int, product_name: str, quantity: int = 1) -> str:
    ...
    if not product:
        cursor.close(); close_db_connection(conn)
        return f"Could not locate '{product_name}'."
    ...
    return f"✅ Added **{quantity}x {product['product_name']}** to your cart."
...
def checkout_cart(user_id: int) -> str:
    ...
    try:
        ...
        if not cart_items: return "Your cart is empty."
            
        for item in cart_items:
            if item['stock'] < item['quantity']:
                return f"Insufficient warehouse stock for {item['product_name']}."
        ...
        conn.commit()
        return f"✅ Order #{order_id} placed successfully for ₹{total}."
    except Exception as e: return f"Checkout failed: {str(e)}"
    finally: cursor.close(); close_db_connection(conn)

def place_order(user_id: int, product_name: str, quantity: int = 1) -> str:
    if user_id == 0: return "Please log in to place orders."
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    ...
    conn.commit(); cursor.close(); close_db_connection(conn)
    return f"✅ Express Order #{order_id} placed successfully for ₹{total}."

def check_order_status(user_id: int, order_id: int) -> str:
    if user_id == 0: return "Please log in to track orders."
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT status, total_amount FROM orders WHERE order_id = %s AND user_id = %s", (order_id, user_id))
    order = cursor.fetchone()
    cursor.close(); close_db_connection(conn)
    return f"Order #{order_id} is currently **{order['status'].upper()}**." if order else f"Order #{order_id} not found."
...
def modify_order(user_id: int, order_id: int, product_name: str, new_quantity: int) -> str:
    ...
    try:
        ...
        if order['status'] != 'pending': return f"Cannot modify: Order #{order_id} is already {order['status']}."
            
        cursor.execute("SELECT * FROM order_items WHERE order_id = %s AND product_name LIKE %s", (order_id, f"%{product_name}%"))
        item = cursor.fetchone()
        if not item: return f"'{product_name}' was not found in Order #{order_id}."
        ...
        if new_quantity == 0:
            ...
            if qty_diff > 0 and cursor.fetchone()['stock'] < qty_diff:
                return "Insufficient warehouse stock to increase quantity."
        ...
        if new_total == 0:
            ...
            conn.commit()
            return f"Item removed. Order #{order_id} is now empty and has been cancelled."
            
        cursor.execute("UPDATE orders SET total_amount = %s WHERE order_id = %s", (new_total, order_id))
        conn.commit()
        return f"✅ Order #{order_id} updated: {product_name} qty is now {new_quantity}. Total: ₹{new_total}."
    except Exception as e: return f"Modification error: {str(e)}"
    finally: cursor.close(); close_db_connection(conn)
...
def cancel_order(user_id: int, order_id: int) -> str:
    ...
    order = cursor.fetchone()
    if not order: return f"Order #{order_id} not found."
    if order['status'] == 'cancelled': return f"Order #{order_id} is already cancelled."
    ...
    conn.commit(); cursor.close(); close_db_connection(conn)
    return f"✅ Order #{order_id} cancelled and items restocked."
...
def cancel_all_orders(user_id: int) -> str:
    ...
    try:
        ...
        if not pending: return "No pending orders to cancel."
        ...
        conn.commit()
        return f"✅ Cancelled {len(pending)} pending order(s) and restocked inventory."
    except Exception as e: return f"Batch cancellation error: {str(e)}"
    finally: cursor.close(); close_db_connection(conn)
...
3. NO HTML: Never attempt to write HTML or formatting tags. The Python tool returns formatted templates.
"""
...
async def process_chat(request: ChatRequest):
    ...
    try:
        ...
        tools_schema = [
            ...
            {"type": "function", "function": {"name": "add_item_to_cart", "description": "Add an item to the shopping cart.", "parameters": {"type": "object", "properties": {"product_name": {"type": "string"}, "quantity": {"type": "integer"}}, "required": ["product_name"]}}},
            {"type": "function", "function": {"name": "place_order", "description": "Express buy a single specific product immediately.", "parameters": {"type": "object", "properties": {"product_name": {"type": "string"}, "quantity": {"type": "integer"}}, "required": ["product_name"]}}},
